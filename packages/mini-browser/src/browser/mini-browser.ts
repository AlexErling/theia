...
export class MiniBrowser extends BaseWidget {
    ...
    protected init(): void {
        ...
        if (startPage) {
            setTimeout(() => this.go(this.props.startPage, true), 500);
    ...
    protected async listenOnContentChange(location: string): Promise<void> {
        if (location.startsWith('file://')) {
            if (await this.fileSystem.exists(location)) {
                ...
                this.fileSystemWatcher.onFilesChanged(e => {
    ...
    protected async go(location: string | undefined, register: boolean = false): Promise<void> {
        if (location) {
            try {
